...
3. Нет физического/MAC-пути для повторного входа после сброса. Не рассчитывай,
...
| `{{SECTOR_SSID}}` | SSID провайдерского сектора | `provider-sector-ssid` |
...
profile и RF-параметры должны быть подтверждены оператором; пример `provider-sector-ssid`
...
```routeros
# Выполняй отдельно и только после явного подтверждения: команда стирает конфигурацию.
...
/interface ethernet set [ find default-name=ether1 ] \
  advertise=10M-half,10M-full,100M-half,100M-full
...
Не переносить в рабочий шаблон без отдельного решения оператора:
`country=debug`, `frequency-mode=superchannel`, `antenna-gain=0`,
`advertise=...1000M...`, `allow-none-crypto=yes`, `forwarding-enabled=remote`
и широкую группу `full`. Это значения из старого исходного листа, а не безопасные
значения для нового устройства. Пароль администратора нужно задать отдельным
замаскированным вводом; он не является частью этой карточки.
