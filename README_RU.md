...
Для прямых tools поштучного approval нет: после входа в режим harness сам выполняет confirmation
token handshake MikroMCP. Опциональные `propose_*` по-прежнему открывают обычный reviewed workflow,
если модель явно выбрала preview.
...
с RouterOS menu path возвращает ограниченные фрагменты с явной меткой недоверенных справочных
данных; URL источников также видны непосредственно в transcript.
